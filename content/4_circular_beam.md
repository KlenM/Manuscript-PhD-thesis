# Circular-beam model with moment matching
## Introduction {#sec:intro_mm}

In ^[sec:validation] we demonstrated that the models based on statistics of beam parameters like beam-wandering model and elliptical-beam model exhibit misspecification bias.
...
In this section we address this problem by introducing a model based on beam-shape parameter statistics augmented with a moment matching procedure.
...
Numerical simulations further show that its empirical distribution is well approximated by a lognormal distribution for all considered in ^[sec:validation] channels.
...
![\label{fig:lognormvalid}Verification of the lognormal approximation for the effective beam size $S$. The plot compares the empirical probability density function obtained from $5\cdot10^5$ numerical beam-propagation simulations (blue histogram) against a fitted lognormal distribution (dashed line). The data corresponds to a moderate turbulence channel with a focused beam ($F_0 = z_\mathrm{ap}$), representing the regime with the worst-case scenario.](moment_matching/MM_Spdf.pdf)
...
The left part of ^[fig:acbks] corresponds to the channel with weak turbulence and $F_0 = z_\mathrm{ap}$.
This case yields the best overall performance of the proposed model.
The improvement over beam shape based models is pronounced, in particular for small aperture radii compared to the beam size $W_\mathrm{LT}$.
Moreover, in this region the proposed model yields the smallest KS statistic among all considered analytical models.
An explicit example of PDT for $R_\mathrm{ap} = 0.45~\text{cm}$ is shown in ^[fig:acbpdt].
...
The channel with moderate turbulence and $F_0 = z_\mathrm{ap}$ yields intermediate results between these two extremes.
...
This removes the model misspecification bias observed in the beam-based models and directly targets the quantities relevant for quantum protocol performance.
...
When analytical approximations of these moments are used, or when experimental noise and systematic errors affect their estimation, the performance of the model can degrade..
