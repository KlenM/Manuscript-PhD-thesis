# Validation of existing models {#sec:validation}
...
In particular, the first and second moments of the transmittance $\left<\eta\right>$ and $\left<\eta^2\right>$ can be obtained^[sec:light_in_turb] from the field correlation functions and can be easily measured experimentally.
Beam shape parameters such as long-term beam-spot radius, short-term beam-spot radius, beam wandering variance, and beam size fluctuations^[sec:light_in_turb] require more involved measurements but remain experimentally accessible.
...
The first important observation is that the physically motivated models (the beam wandering model and the elliptical beam model) being defined in terms of beam shape statistics, exhibit biased transmittance moments.
