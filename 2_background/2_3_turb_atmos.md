...
## Energy cascade model of turbulence
...
Let's consider the structure function of the velocity field -- ensemble average of the square of longitudinal velocity difference at two points in space. 
...
![fig:psd_scales](file:///home/klen/syncthing/desktop/physics/phd/thesis/src/generated/psd_scales.svg "The model of turbulent atmosphere and its power spectral density for Kolmogorov model blue and Modified von Karmal model green")

The ~fig:psd_scales~ summarises the model of atmospheric turbulence. 
...
>The frozen turbulence hypothesis, analogous to observing clouds drifting across the sky, assumes that turbulent structures remain statistically frozen while being advected by mean wind velocity. This approximation enables transformation between temporal and spatial turbulence statistics, facilitating experimental validation of theoretical predictions.
>- is this advection  motion?
...
The second is the mentioned above the inertial time $T_I \sim r/v_r$ `\ref{sec:turbtheory}`. 
...
Then in the case when the advection time is much smaller than inertial time $T_v/T_I = v_r/v \ll 1$  we can assume that
$$n(\mathbf{r},\,t+\tau)=n(\mathbf{r}+\mathbf{v}\,\tau,\,t)$$
where $\mathbf{v}: |\mathbf{v}|=v$, and say that Taylor frozen turbulence hypothesis is hold.
